...
def aod_imputation_RF(date,
    ...
    unimputed_xr['ndvi'] = unimputed_xr['ndvi'].interpolate_na(dim='x',
                                                               method='nearest',
                                                               fill_value='extrapolate')

    unimputed_3darray = np.array([
        unimputed_xr['ndvi'],  # 0
        unimputed_xr['aod_buffer_047'],  # 1
        unimputed_xr['aod_buffer_055'],  # 2
        unimputed_xr['dem'][0],  # 3
        unimputed_xr['daymet_dayl'],  # 4
        unimputed_xr['daymet_prcp'],  # 5
        unimputed_xr['daymet_srad'],  # 6
        unimputed_xr['daymet_swe'],  # 7
        unimputed_xr['gridmet_th'],  # 8
        unimputed_xr['daymet_tmax'],  # 9
        unimputed_xr['daymet_tmin'],  # 10
        unimputed_xr['daymet_vp'],  # 11
        unimputed_xr['gridmet_vs'],  # 12
        unimputed_xr['daymet_lat'],  # 13
        unimputed_xr['daymet_lon'],  # 14
        unimputed_xr['wildfire_smoke'],  # 15
        unimputed_xr['year'],  # 16
        unimputed_xr['day_sin'],  # 17
        unimputed_xr['day_cos'],  # 18
        unimputed_xr['month_sin'],  # 19
        unimputed_xr['month_cos'],  # 20
    ])
    ...
    model_047_wbuffer_idx = (~np.isnan(unimputed_3darray[1])) & (~np.isnan(unimputed_3darray[2])) \
                            & (~np.isnan(unimputed_3darray[4])) \
                            & (~np.isnan(unimputed_3darray[8])) & (np.isnan(aod_3darray[0]))

    model_055_wbuffer_idx = (~np.isnan(unimputed_3darray[1])) & (~np.isnan(unimputed_3darray[2])) \
                            & (~np.isnan(unimputed_3darray[4])) \
                            & (~np.isnan(unimputed_3darray[8])) & (np.isnan(aod_3darray[1]))

    # Get idx of coordinates without buffer
    model_047_wobuffer_idx = (np.isnan(unimputed_3darray[1])) & (np.isnan(unimputed_3darray[2])) \
                             & (~np.isnan(unimputed_3darray[4])) \
                             & (~np.isnan(unimputed_3darray[8])) & (np.isnan(aod_3darray[0]))

    model_055_wobuffer_idx = (np.isnan(unimputed_3darray[1])) & (np.isnan(unimputed_3darray[2])) \
                             & (~np.isnan(unimputed_3darray[4])) \
                             & (~np.isnan(unimputed_3darray[8])) & (np.isnan(aod_3darray[1]))
    ...
    nobuffer_col = np.delete(np.arange(unimputed_3darray.shape[0]), [1, 2])
    ...
    unimputed_xr.to_netcdf(path=f'./outputs/merged_{str(date)}_with_imputation.nc',
                           mode='w',)
    print('Exporting Finished')
    os.remove(f'./outputs/merged_{str(date)}_wo_imputation.nc')

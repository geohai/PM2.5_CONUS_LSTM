...
def train_RF(train_df, buffer_avg=True, target="047", validation=False):
    ...
    # Refit the best regressor to full dataset
    X_full = pd.concat([X_train, X_val])
    y_full = pd.concat([y_train, y_val])
    best_xgb_regressor = xgb_regressor_cv.best_estimator_
    best_xgb_regressor.fit(X_full, y_full.values.ravel())

    full_r2_xgb = round(best_xgb_regressor.score(X_full, y_full), 2)
    print('Training R2 score of XGBoost is {}'.format(train_r2_xgb))
    y_full_predicted_xgb = best_xgb_regressor.predict(X_full)
    rmse_full_xgb = (np.sqrt(mean_squared_error(y_full, y_full_predicted_xgb)))
    print('RMSE on the training set for the XGBoost model is: {}'.format(rmse_full_xgb))
    mbe_full_xgb = np.mean(y_full_predicted_xgb - y_full.values.squeeze())
    print("MBE on training set is for the XGBoost model is: {}".format(mbe_full_xgb))
